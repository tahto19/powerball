...
import { Alert, Popover, useMediaQuery, useTheme } from "@mui/material";
...
export default function SideMenu() {
  ...
  const [anchorElForSetupUser, setAnchorElForSetupUser] =
    React.useState<null | HTMLElement>(null);

  const getSetUser = React.useRef(null);
  const handleClose = () => {
    setAnchorElForSetupUser(null);
  };
  React.useEffect(() => {
    if (token) {
      if (
        ...
        setAnchorElForSetupUser(getSetUser.current);
      else setAnchorElForSetupUser(null);
    }
  }, [getSetUser, data]);
  return (
    <>
      ...
      >
        ...
        >
          <Avatar
            ...
            alt="Riley Carter"
            src={
              data.fileInfo && data.fileInfo.id
                ? endpoint + data.fileInfo.id + `?t=${refreshKey}`
                : ""
            }
          ...
          />
          <Popover
            open={!!anchorElForSetupUser}
            anchorEl={anchorElForSetupUser}
            anchorOrigin={{
              vertical: "top",
              horizontal: "left",
            }}
            onClose={handleClose}
          >
            <Alert severity="info">Your Account needs setup</Alert>
          </Popover>
