...
const MyDialog = ({
  ...
  const [prize_List, setPrizeList] =
    useState<PrizeListAll>(initialPrizeListData);
  ...
  const handleSubmit = async (e: React.FocusEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload: PayloadState = {
        formData,
        newPrizeList: selectedPrize.map((x) => ({
          id: Number(x.id),
          value: x.value,
  ...
  const handlePrizeNameChange = (
    ...
    const amount = prize_List.list.filter(
