...
const Index = () => {
  ...
  const { loading, list, offset, limit, sort, count } = useAppSelector((state: RootState) => state.admin)
  ...
  useEffect(() => {
    ...
    setPagination((prev) => { return { page: offset, pageSize: limit } })
  }, [loading])
...
const handleViewAction = (row: PrizeState) => {
  ...
  const headers: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params: { value: any }) => {
        return params.value ? <CheckIcon sx={{ color: 'green' }}/> : <CloseIcon sx={{ color: 'red' }}/>;
      },
    }
  ]
  ...
  return (
    <Grid2 container spacing={2} columns={12}>
      ...
      <MyDialog open={open} onClose={onClose} data={data} dialogType={dialogType}/>
    </Grid2>
  );
};
